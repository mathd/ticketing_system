...
//      the ADR-004 hours tier (proves migration 0003 + handler + scoping);
...
// are migrations (0002, 0003) — deterministic by construction, so the
// assertions name a specific venue.
...
func TestBackofficeVenueReadHoursTier(t *testing.T) {
	code, body := get(t,
		gatewayURL+"/api/catalog/public/venues?organizer_id="+organizerID, nil)
	if code != http.StatusOK {
		t.Fatalf("venue read: status %d: %s", code, body)
	}
	// ADR-004 hours tier — long-lived venue geometry, not the events minutes tier.
	// (Header is asserted on the raw response below; get() drops it, so re-fetch.)
	req, _ := http.NewRequest(http.MethodGet,
		gatewayURL+"/api/catalog/public/venues?organizer_id="+organizerID, nil)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("venue read (headers): %v", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if got := resp.Header.Get("Cache-Control"); got != "public, max-age=3600, s-maxage=3600" {
