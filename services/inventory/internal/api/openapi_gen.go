...
const (
	...
	AvailabilityUnavailable       ErrorCode = "availability_unavailable"
	...
	SeatOccupancyUnavailable      ErrorCode = "seat_occupancy_unavailable"
...
type Error struct {
	// Channel The offending allocation's raw channel code, sent by the two per-row allocation refusals — `allocation_cap_below_consumption` (TKT-244) and `allocation_window_reversed` (TKT-307) — which name a row rather than the whole set.
