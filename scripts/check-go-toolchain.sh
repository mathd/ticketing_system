...
anchors() { # emits "<label>\t<version>" per declared pin
  local root="$1" f v
  awk '$1=="go"{print "go.work\t" $2; exit}' "$root/go.work"
  while IFS= read -r f; do
    awk -v l="$f" '$1=="go"{print l "\t" $2; exit}' "$root/$f"
  ...
  for f in "$root"/.github/workflows/*.y*ml; do
    sed -nE 's/.*go-version: "([0-9]+\.[0-9]+(\.[0-9]+)?)".*/\1/p' "$f" |
...
selftest() {
  ...
  case_ 1 "a single go.mod left behind is refused" \
    'active Go\t1.27.0\ngolangci-lint\t1.27.0\ngo.work\t1.27.0\nsmoke/go.mod\t1.26.6\n'
  case_ 1 "the builder image left behind is refused" \
    'active Go\t1.27.0\ngolangci-lint\t1.27.0\ngo.work\t1.27.0\nbuild/go.Dockerfile\t1.26.6\n'
  case_ 1 "a workflow pin left behind is refused" \
    'active Go\t1.27.0\ngolangci-lint\t1.27.0\ngo.work\t1.27.0\n.github/workflows/hermetic.yaml\t1.26.6\n'
...
ROOT="$(git rev-parse --show-toplevel)"
{
  ...
  printf 'golangci-lint\t%s\n' "$("$LINTER" --version | sed -nE 's/.*built with go([0-9.]+).*/\1/p')"
  anchors "$ROOT"
} | converge
echo "go toolchain: every declared pin, the active Go and the linter's build agree on $(minor "$(go env GOVERSION | sed 's/^go//')")"
