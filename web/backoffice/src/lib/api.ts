...
function wholeNumber(v: unknown, field: string): number {
  ...
  // silently rounds anything past 2^53 — 9007199254740993 minor units arrives
  // as ...992, still passes an isInteger check, and would be rendered to an
  // operator as commerce's exact refund amount (ai-review pass 1). The money
  // that moved is server-derived and unaffected; the CLAIM on screen would be
  // wrong, and a money figure that is quietly wrong is worse than one this
  // console admits it cannot show.
