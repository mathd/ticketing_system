...
// point.
//
// This deliberately mirrors the design in web/backoffice/src/lib/session.ts and
// ADR-042. The one departure is the cookie path; see SESSION_COOKIE_PATH.
//
 ...
 * accumulate entries without limit, while the sweep grows more expensive.
 *
 ...
 * principals, each entitled to five sessions that live out the full TTL. Unlike
 * operator-provisioned staff, public customers have no bounded headcount.
 *
 * **At capacity this REFUSES a new session; it does not evict a live one.** Eviction
 * would turn memory pressure into a targeted availability attack: an actor can fill
 * the map with public registrations and displace another customer's live session.
 *
 ...
 * because expiry-on-read only collects a token that is presented.
 *
 ...
 *
 * A non-decreasing floor over `Date.now()` does not close that gap. The floor only
 * rises during a call, while the unsafe interval occurs when no call observed the
 * clock before it moved backward.
 *
 ...
 * payment button.
 *
 * This is a BEST-EFFORT UPPER BOUND, not synchronization. The subtraction mixes
 * this host's wall clock with commerce's, so a storefront clock running behind
 * commerce's still leaves a window where the session outlives the assertion. **Commerce remains
...
function assertionLifetimeMs(assertion: string, wallClockNow: number): number | undefined {
  ...
  // commerce never minted.
  if (!field || !/^\d+$/.test(field)) return undefined;
  return Number(field) * 1000 - wallClockNow;
}

export interface CustomerSessionStore {
  create(principal: CustomerPrincipal): string;
  lookup(token: string): CustomerPrincipal | undefined;
  destroy(token: string): void;
  destroyAllForCustomer(customerId: string): number;
}

export type CustomerSessionStoreOptions = {
 ...
 * unit tests create their own instances so state and capacity settings cannot leak.
 */
export function createSessionStore(
  {
    ...
    wallClock = Date.now,
  }: CustomerSessionStoreOptions,
): CustomerSessionStore {
  if (
    !Number.isInteger(maxSessionsTotal) ||
    maxSessionsTotal < MAX_SESSIONS_PER_CUSTOMER + 1
  ) {
    throw new RangeError(
      `session cap must be an integer >= ${MAX_SESSIONS_PER_CUSTOMER + 1}, got ${maxSessionsTotal}`,
    );
  }

  const sessions = new Map<string, Entry>();

  function createSession(principal: CustomerPrincipal): string {
    ...
    // a customer who is already at their own five-session cap
    ...
    const assertionMs = assertionLifetimeMs(principal.assertion, wallClock());
    const lifetime =
  ...
  function lookupSession(token: string): CustomerPrincipal | undefined {
   ...
   * commerce.
   */
  function destroyAllSessionsForCustomer(customerId: string): number {
    let destroyed = 0;
    for (const [token, entry] of sessions) {
  ...
  return {
    ...
    destroyAllForCustomer: destroyAllSessionsForCustomer,
  };
}

/** The session store owned by this storefront process. */
export const sessionStore = createSessionStore({ maxSessionsTotal: MAX_SESSIONS_TOTAL });
