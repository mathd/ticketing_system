...
> **Custom agentic workflow — deliberately richer than the org standard.** The agent stages (`Planning`, `Building`) are **first-class Jira statuses**, not just labels, because the label axis alone isn't visible enough for human + AI co-work — that visibility is the point of this pipeline. **Re:sources** creates the bare Jira project + Confluence space via a ServiceNow request (you can't self-create them); **you** (Team Lead admin) then build this workflow/board yourself — the request form has no workflow fields, so nothing to negotiate. Two things are kept from the standard because the business wants them: the **PO Review** status (gate 4) and the transversal **BLOCKED** status. `references/setup.md` §1 has the details.
...
|---|--------|-------------|------------------------------|-----------|
| 1 | `Backlog` | issue created | Write **COS** (Conditions of Success — Moov AI's term for acceptance criteria) + scope; suggest `risk:low` if trivial. **Every pipeline ticket gets the context-mémo bake** (`decomposition.md` § context-mémo) — standalone tickets too, not just PRD children. **Raw idea →** explore first (`exploration.md`). **PRD/multi-ticket →** decompose (`decomposition.md`): epic + children + dependency links + context-mémo bake, ending with the readiness verdict. | ⛔ human → `Ready` |
...
| 6 | `Done` | PO accepted | Remove `needs:human`; verify PR merged + the **Moov AI DoD** is satisfied (see `references/setup.md`); post the metrics comment (durations from the Jira changelog); promote reusable learnings; delete branch. | — |
...
- **Cross-ticket memory.** Reusable learnings (codebase patterns, gotchas) go to a dedicated **"🧠 Agent memory" Jira issue** (or the team decision registry). When a pattern proves out, promote it per Moov AI's **"3 houses" rule** (`references/setup.md`): **technical** standards → **py-moov** (`github.com/moovai/py-moov`, via PR), **process** learnings → Confluence, repo-local guidance → `AGENTS.md` via a normal gated ticket. Working memory ≠ gospel.
