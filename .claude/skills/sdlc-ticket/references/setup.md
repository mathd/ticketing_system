...
The Atlassian MCP creates issues, links, comments, and Confluence pages — but **not Jira projects, workflows, boards, components, or Confluence spaces**. So setup splits three ways: **Re:sources** creates the empty Jira project + Confluence space (ServiceNow request); the **Team Lead** (project admin) configures the workflow, board, components, access, and connections; then the **agent** resolves IDs and writes the config.

> **Client-owned environment?** The Re:sources path below is Moov AI's internal route. When the *client* owns Jira/Confluence/the repo, confirm the **prerequisites** up front, with a client admin partner if the consultant lacks admin: project/workflow creation, entity-property write access (else the description fallback), branch protection, cross-model review allowed, and who owns each gate (PO is often client-side; if the client owns the repo, the merge gate may be theirs). If a prerequisite can't be met — notably cross-model review — this SDLC isn't a fit for that engagement; don't run a degraded variant.

## 1. Jira project + Confluence space (Re:sources request, then your board setup)

**Source of truth:** Moov·Base — [*Création nouveau projet dans Jira*](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1319993877). Follow that page; the essentials:

- **Re:sources creates the empty shells** via ServiceNow ([catalog item](https://support.publicisgroupe.net/pgsp?id=sc_cat_item&sys_id=6400ee7187580394027463170cbb3538)): agency **ReSources-Global**, type **New Project Setup**, application **Jira** — this provisions **both** the Jira project and the Confluence space, with the **name + key you give in the request**. That request is Re:sources' *entire* role; **everything else on the Moov·Base page (project settings, access, connections, workflow, issue types, columns) is yours** to configure afterward as Team Lead admin.
...
**Who builds this:** you do. Re:sources creates only the bare Jira project + Confluence space — the ServiceNow form carries no workflow/board options — so the custom statuses, transitions, board columns, and components above are **yours to configure** as Team Lead admin. No approval or deviation request is involved; Re:sources never touches the workflow.

### Moov AI delivery standard — sources of truth

This pipeline sits **inside** Moov AI's standard delivery workflow. Defer to these pages; don't duplicate or contradict them:

- [Workflow de livraison](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1319994067) — the canonical sprint flow and the authoritative Jira status table.
- [Definition of Done (DoD)](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1319994151) — **`Done` is defined here, not in this skill.** The local gate is necessary but not sufficient; a ticket is Done only when the DoD is met.
- [Guide Jira – Création de tickets](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1320059419) — ticket-creation conventions.
- [Cadre de livraison](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1297154269) + [template](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1321009172) — the project cadre (6 sections); complete the [checklist de départ](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1320059114) first.

**Semantics the agent must respect** (from *Workflow de livraison*):
- **COS = Conditions of Success** (Moov AI's term for acceptance criteria; *critères d'acceptation* on the FR pages). Write COS on every ticket; the PO gate validates them. This skill's "AC" = COS.
...
**Where things live — the "3 houses" rule** ([Cadre](https://tools-resources-global.atlassian.net/wiki/spaces/MoovBase/pages/1297154269)): code + technical standards → **py-moov** ([github.com/moovai/py-moov](https://github.com/moovai/py-moov); agent skills in py-moov-cli); process/org/project docs → **Confluence**; solution definition + client deliverables → **SharePoint**. So promote reusable **technical** learnings to py-moov via PR and **process** learnings to Confluence — not just a Jira memo.
...
## 2. Confluence space (created by Re:sources with the project)

Re:sources provisions the space alongside the Jira project (§1). Record its key and connect it to the Jira project. Epic PRDs become pages here, linked from the epic; story/task "PRDs" stay in the issue description.
