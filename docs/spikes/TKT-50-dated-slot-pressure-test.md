...
- **Access event shape:** the single `redeemed` terminal event generalizes to an append-only **entry/exit stream**. Concretely US-009's downstream (not this spike) relaxes `lifecycle_events` for multi-entry slots: `event_type` gains `entry` / `exit`, and the `UNIQUE(ticket_id, event_type)` guarantee is **kept for `redeemed`** (performances) and **not applied to `entry`/`exit`** (multi-entry). "Redeemed exactly once" stays a hard DB guarantee for the single-entry case; it is not weakened — a different event type carries the multi-entry case. What `redeemed` *means* is pinned per mode: `single` → first (and only) scan; `multi`/`count_limited` → an `entry` event, with `redeemed` reserved for single.
...
**Cut — the hard edge.** A cut *below* `confirmed_quantity` cannot be represented by simply lowering `capacity`, because that violates ADR-010's pool invariant `confirmed + held ≤ capacity`. Three candidate representations were considered:

1. **Reject-below-confirmed** — the pool refuses a capacity event that would drop `capacity < confirmed_quantity`; the operator can only cut down to current sold. Simplest, preserves the invariant by construction, but can't express "stop selling, we're over" after the fact.
2. **Separate requested vs effective capacity** — store the operator's requested cap independent of the enforced floor (`effective = max(requested, confirmed_quantity)`). Expressive but adds a second number to reason about and to trace.
...
**Decision for US-009's schema:** capacity mutation stays entirely in Inventory (no catalog schema surface), and the **claim path already gives us the behaviour for free**: a cut is applied as `capacity := max(new, confirmed_quantity)` (option 1's floor) **plus** the `sales_halted` semantics of option 3 when the operator's intent is "below current occupancy" — i.e. *block new claims, never force-release a confirmed claim*. A confirmed admission is a promise already made; capacity policy is forward-only. This needs **no catalog attribute** — US-009 adds nothing here beyond ensuring the publication/capacity event envelope can carry a capacity delta. The concrete inventory representation (floor vs flag vs both) is an **inventory-side** decision owed to the capacity-adjustment ticket, not to US-009's slot schema; US-009 must only avoid putting a mutable count in Catalog.
...
- **State machine:** `open —(close)→ closed —(reopen?)→ open`. Reopen **is** allowed (a weather hold that lifts) and is why closure is a toggle attribute, not a terminal transition. Archive still applies independently after the fact.
...
⚠️ **One scoped exception → ADR-013.** Live **concurrent-occupancy capping** (a fire-code "no more than N people physically inside right now" limit, decremented on exit) is **not** the same primitive as monotonic no-oversell reservation. Reservation counts *admissions sold* (only rises until release/expiry); occupancy counts *bodies present* (rises and falls). Forcing occupancy into the claim core would fork it. It is carved out: **not required for US-009**, which stores only slot identity/kind/hours/re-entry/closure; live occupancy, if ever built, attaches by `slot_id` in Access or a future capacity-control component — **never** as a live count in Catalog or in Inventory claims. See ADR-013.
