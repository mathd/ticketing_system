...
```
make check-required-env    # every ${VAR:?} in compose.yaml is generated by env-bootstrap.sh
```

It bootstraps into a throwaway sandbox (never your `.env`), prints names and never values, and runs
in CI inside the `gate-selftest` job, where a seeded deletion proves it can still fail. It is
deliberately **not** in `make check` — that gate never starts the stack it protects. When you add a
`${VAR:?}` to `compose.yaml`, add the variable to `env-bootstrap.sh` in the same change.
